...
class Wbty_formsHelperAjax
{
	...
	public function link_html($link_name ='', $key = '{user_groups}', $link, $check_trashed = false) {
		...
		if (is_array($link) && isset($link['form'])) {
			$type_options = $link['type_options'];
		...
		if ($type_options) {
			$model = $this->getLinkModel('form');
			$form = $model->getForm();
			
			$fieldset = $form->getFieldset(strtolower($field_type));

			foreach ($fieldset as $field) {
				$option = $type_options[$field->__get('fieldname')];
				if (!$option) {
					continue;
				}
				if ($field->__get('type') != 'Hidden') {
					...
					// our field types have a displayValue function, especially for sql fields.
					$string .= '
					<li><span class="value-label">'.strip_tags(str_replace('<span class="star">&#160;*</span>', '', $field->__get('label'))).':</span> <span class="value">'. ( $option->value ) .'</span></li>';
		        }
	         	$values[$field->__get('name')] = $option->value;
			}
		}
