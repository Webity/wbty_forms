 ...
 * Wbty_forms model.
 */
class Wbty_formsModelfield_type extends WbtyModelAdmin
{
	protected $text_prefix = 'com_wbty_forms';
	protected $com_name = 'wbty_forms';
	protected $list_name = 'field_types';

	public function getTable($type = 'field_types', $prefix = 'Wbty_formsTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform', $key=0)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_forms.field_type.'.$control.'.'.$key, 'field_type', array('control' => $control, 'load_data' => $loadData, 'key'=>$key));
		if (empty($form)) {
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_forms.edit.field_type.data', array());
	...
	{
		if ($item['field_type'] = parent::getItem($pk)) {
