<?php
/**
 * @version     0.2.0
 * @package     com_wbty_forms
 * @copyright   Copyright (C) 2012-2013. All rights reserved.
 * @license     GNU General Public License version 2 or later; see LICENSE.txt
 * @author      Webity <info@makethewebwork.com> - http://www.makethewebwork.com
 */

// No direct access
defined('_JEXEC') or die;

jimport('wbty_components.controllers.wbtycontrollerform');

/**
 * form controller class.
 */
class Wbty_formsControllerForm extends WbtyControllerForm
{
	protected $view_list = 'forms';
    protected $view_form = 'form';
    protected $com_name = 'wbty_forms';

    function __construct() {
        parent::__construct();
		
		$this->_model = $this->getModel();
    }
	
	function back() {
		$this->setRedirect(
			JRoute::_(
				'index.php?option=' . $this->option . '&view=' . $this->view_list
				. $this->getRedirectToListAppend(), false
			)
		);
	}
	
	public function &getModel($name = 'form', $prefix = 'Wbty_formsModel')
	{
		$model = parent::getModel($name, $prefix, array('ignore_request' => true));
		return $model;
	}

	function validateForm() {
		$model = $this->getModel();
		$model->setModelState();
		$array['status'] = $model->getSave();
		if (!is_bool($array['status'])) {
			$array['redirect'] = JRoute::_($array['status']);
		}

		$item = $model->getItem();
		if ($item->thank_you_message) {
			$array['html'] = $item->thank_you_message;
		}
		echo json_encode($array);
		exit();
	}
	
}