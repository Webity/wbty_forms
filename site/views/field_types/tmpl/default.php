...
<form action="<?php echo JRoute::_('index.php?option=com_wbty_forms&view=field_types'); ?>" method="post" name="adminForm" id="adminForm" class="form-inline">
	...
	<h2 class="title centered"><?php echo JText::_('Field_Types'); ?></h2>
	<div class="clr"></div>
	...
	<table class="adminlist table table-striped table-bordered">
		<thead>
			<tr>
				...
				<th></th>
					...
					<th>
						<?php echo JHtml::_('grid.sort',  'COM_WBTY_FORMS_FORM_LBL_FIELD_TYPES_NAME', '.name', $listDirn, $listOrder); ?>
					</th>
				...
				<th width="10%">
					...
					<?php if ($canOrder && $saveOrder) :?>
						<?php echo JHtml::_('grid.order',  $this->items, 'filesave.png', 'field_types.saveorder'); ?>
					<?php endif; ?>
				</th>
                <?php } ?>
		...
		<?php foreach ($this->items as $i => $item) :
			...
			<tr class="row<?php echo $i % 2; ?>">
                ...
                <td class="center">
                	<div class="btn-group">
                      ...
                      <ul class="dropdown-menu">
                        <?php
                        echo '<li><a href="index.php?option=com_wbty_forms&view=field_type&layout=default&id='.$item->id.'">View</a></li>';
						if ($canEdit && $this->state->get('filter.state') != -2) {
	                        echo '<li><a href="index.php?option=com_wbty_forms&task=field_type.edit&id='.$item->id.'">Edit</a></li>';
						}
                        ?>
                ...
                <?php if (0&&isset($this->items[0]->state)) { ?>
				    <td class="center">
					    <?php echo JHtml::_('jgrid.published', $item->state, $i, 'field_types.', $canChange, 'cb'); ?>
				    </td>
                <?php } ?>
                <?php if (0&&isset($this->items[0]->ordering)) { ?>
				    <td class="order">
					    <?php if ($canChange) : ?>
						    <?php if ($saveOrder) :?>
							    <?php if ($listDirn == 'asc') : ?>
								    <span><?php echo $this->pagination->orderUpIcon($i, true, 'field_types.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'field_types.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							    <?php elseif ($listDirn == 'desc') : ?>
								    <span><?php echo $this->pagination->orderUpIcon($i, true, 'field_types.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								    <span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, true, 'field_types.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							    <?php endif; ?>
						    <?php endif; ?>
