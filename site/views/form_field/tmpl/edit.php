...
window.addEvent('domready', function(){
    ...
    document.form_fieldvalidator = document.formvalidator;
    jQuery('#form_field-form .toolbar-list a').each(function() {
    	$(this).attr('data-onclick', $(this).attr('onclick')).attr('onclick','');
    });
    jQuery('#form_field-form .toolbar-list a').click(function() { 
    	Joomla.submitbutton = document.form_fieldsubmitbutton;
...
Joomla.submitbutton = function(task)
{
    ...
    if (task == 'form_field.cancel' || document.form_fieldvalidator.isValid(document.id('form_field-form'))) {
        Joomla.submitform(task, document.getElementById('form_field-form'));
    }
    else {
        alert('<?php echo $this->escape(JText::_('JGLOBAL_VALIDATION_FORM_FAILED'));?>');
    }
}
document.form_fieldsubmitbutton = Joomla.submitbutton;
<?php
...
<form action="<?php echo JRoute::_('index.php?option=com_wbty_forms&form_id='.JRequest::getCmd('form_id').'&layout=edit&id='.(int) $this->item->id); ?>" method="post" name="adminForm" id="form_field-form" class="formview form-validate form-horizontal">
    <?php echo $this->addToolbar(); ?>
    <div class="clr"></div>
    <div class="row-fluid">
    	<div class="span6">
    		<fieldset class="adminform parentform" data-controller="form_field" data-task="form_field.ajax_save">
    			<legend><?php echo JText::_('COM_WBTY_FORMS_LEGEND_FORM_FIELD'); ?></legend>
                <div class="items">
                    <?php 
                        foreach($this->form->getFieldset('form_field') as $field):
                    ...
                    <div class="control-group"> 
                        <div class="controls">
                            <span class="btn btn-success save-primary"><i class="icon-ok"></i> Save Form_Field Info</span>
                        </div>
                    </div>
                </div>
