	...
	jQuery.ajax({
		...
		success: function(msg) {
			if (msg.error) {
				...
				if (msg.id) {
					if (jQuery(this).closest('fieldset').find('.id').length > 0) {
						jQuery(this).closest('fieldset').find('.id').val(msg.id);
					} else {
						jQuery('.id').first().val(msg.id);
					}
				}
			}
			if (msg.token) {
...
jQuery(document).ready(function($) {
	...
	$('#toolbar-save button, #toolbar-save a').attr('onclick', '');
	$('#toolbar-save button, #toolbar-save a').click(function(e) {
		e.preventDefault();
		...
		// save any inline edit forms
		$('.inline-edit-save-btn').trigger('click');

		// save primary
		data = $('fieldset.parentform').serializeArray();
		token = new Object;
		token.name = ((typeof window.token === 'undefined') ? window.token : $('fieldset.parentform').closest('form').find('input').last().attr('name'));
		token.value = 1;
		data.push(token);

		$(this).after('<img src="'+window.juri_root+'/media/wbty_forms/img/load.gif" class="loading" />');
		task = $('fieldset.parentform').attr('data-task');
		url = 'index.php?option=com_wbty_forms&task='+task;
	...
	hidden_forms = $('#hidden-forms').html();
	$('#hidden-forms').remove();
	...
	$(document).on('click', '.inline-edit-trigger', function(e) {
		...
		url = window.juri_base + 'index.php?option=com_wbty_forms&task='+ $this.attr('data-controller') + '.edit';
		...
		$(document).trigger('inline-edit-trigger-after');
	...
	$('body').on('click', '.inline-edit-save-btn', function(e) {
		...
		url = window.juri_base + 'index.php?option=com_wbty_forms&task='+ $fields.attr('data-controller') + '.ajax_save';
		
		if ($fields.attr('data-append')) {
			url = url + '&' + $fields.attr('data-append');
		}
		
		$(this).closest('.edit-form').after($('<p style="text-align:center;" data-update="'+$(this).closest('.edit-form').attr('data-update')+'" data-fieldset="'+$(this).closest('.edit-form').attr('data-fieldset')+'"><img src="'+window.juri_root+'media/wbty_forms/img/load.gif" /></p>'));
